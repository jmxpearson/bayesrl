// Model 3:
// Model 1 + separate learning rates for each run
    ...
    for (idx in 1:N) {
        ...
        if (outcome[idx] >= 0) {
                ...
                if (trial[idx] < Ntrial) {  // update action values for next trial
                    ...
                    Q[sub[idx], trial[idx] + 1, unchosen[idx]] <- Q[sub[idx], trial[idx], unchosen[idx]] + alpha[sub[idx], run[idx]] * Delta[sub[idx], trial[idx], unchosen[idx]];
                }
        }
    }
}
